...
struct s2n_composite_cipher {
  ...
  int (*initial_hmac)(struct s2n_session_key *key, uint8_t *sequence_number, uint8_t content_type, uint16_t protocol_version, uint16_t payload_and_eiv_len, int *extra);
};
...
extern const struct x2n_cipher s2n_aes128_sha256;
