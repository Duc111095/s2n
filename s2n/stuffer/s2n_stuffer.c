...
S2N_RESULT s2n_stuffer_reservation_validate(const struct s2n_stuffer_reservation* reservation) {
	...
	RESULT_ENSURE_LTE(reserve_obj.write_cursor, UNIT32_MAX - reserve_obj.length);
	RESULT_ENSURE_LTE(reserve_obj.write_cursor + reserve_obj.length, stuffer_obj.write_cursor);
	RESULT_ENSURE_LTE(reserve_obj.write_cursor, stuffer_obj.read_cursor);
	return S2N_RESULT_OK;
}
...
{
	POSIX_PRECONDITION(s2n_stuffer_validate(stuffer));
	POSIX_GUARD(s2n_stuffer_reverse_space(stuffer, n));
...
int s2n_stuffer_writev_bytes(struct s2n_stuffer* stuffer, const struct iovec* io, size_t iov_count, uint32_t offs,
	uint32_t size)
{
	POSIX_PRECONDITION(s2n_stuffer_validate(stuffer));
	...
	for (size_t i = 0; i < iov_count; i++) {
		...
		uint32_t iov_size_to_take = S2N_MIN(iov_len, size_left);
