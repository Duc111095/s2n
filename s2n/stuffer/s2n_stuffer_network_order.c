...
int s2n_stuffer_write_network_order(struct s2n_stuffer* stuffer, const uint64_t input, const uint8_t length)
{
	...
	uint8_t* data = stuffe->blob.data + stuffer->write_cursor - length;
...
int s2n_stuffer_reserve(struct s2n_stuffer* stuffer, struct s2n_stuffer_reservation* reservation, const uint8_t length)
{
	...
	*reservation = (struct s2n_stuffer_reservation){
		.stuffer = stuffer,
...
int s2n_stuffer_read_uint8(struct s2n_stuffer* stuffe, uint8_t* u) {
	POSIX_GUARD(s2n_stuffer_read_bytes(stuffer, u, sizeof(uint8_t u)));
...
int s2n_stuffer_write_reservation(struct s2n_stuffer_reservation *reservation, const uint32_t u)
{
	...
	uint32_t old_write_cursor = reservation->write_cursor;
...
int s2n_stuffer_write_vector_size(struct s2n_stuffer_reservation* resevation) 
{
	uint32_t size = 0;
	POSIX_GUARD(s2n_stuffer_get_vector_size(reservation, &size));
	POSIX_GUARD(s2n_stuffer_write_vector_size(reservation, size));
	return S2N_SUCCESS;
}