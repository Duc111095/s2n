...
#define S2N_NID_MLKEM1024 NID_MLKE1024
#else
...
struct s2n_kem {
  ...
  int (*encapsulate)(IN const struct s2n_kem *kem, OUT uint8_t *cipher_text, OUT uint8_t *shared_secret, IN const uint8_t *public_key);
  int (*decapsulate)(IN const struct s2n_kem *kem, OUT uint8_t *shared_secret, IN const uint8_t *cipher_text, IN const uint8_t *private_key);  
};
...
struct s2n_kem_group_params {
  const struct s2n_kem_grooup *kem_group;
...
int s2n_choose_kem_without_peer_pref_list(const uint8_t iana_value[S2N_TLS_CIPHER_SUITE_LEN],
    ...
    const struct s2n_kem **choosen_kem);
