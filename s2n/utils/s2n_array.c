...
{
	DEFER_CLEANUP(struct s2n_blob_mem = { 0 }, s2n_free);
...
S2N_RESULT s2n_array_remove(struct s2n_array* array, uint32_t idx)
{
	...
	if (idx != array->len - 1) {
		...
		memmove(array->mem_data + array->element_size * idx,
			array->mem_data + array->element_size * (idx + 1),
			size);
	}
	array->len--;
